  ...
  return (
    <div>
      ...
      <div className="mt-1 relative rounded-md shadow-sm">
        ...
        />
        {type === "password" && formData?.password?.length > 0 && (
            showPassword ? (
                <Eye className="absolute top-1/2 right-4 -translate-y-1/2 " size={18} onClick={() => setShowPassword(!showPassword)}/>
            ):(
                <EyeOff className="absolute top-1/2 right-4 -translate-y-1/2  " size={18} onClick={() => setShowPassword(!showPassword)}/>
            )
        )}
